...
export const isUserAdmin = async (email: string): Promise<boolean> => {
  ...
  try {
    const usersCollection = collection(db, 'users');
    const q = query(usersCollection, where('email', '==', email), where('role', '==', 'admin'));
    const querySnapshot = await getDocs(q);
...
export const getCurrentUserData = async (): Promise<User | null> => {
  if (useMockData) return getCurrentUser();
  
  // This would typically use the authenticated user's ID from Firebase Auth
  // For now, we'll use the admin email as a fallback
  const adminEmail = import.meta.env.VITE_ADMIN_EMAIL || 'julio.v@earthrated.com';
